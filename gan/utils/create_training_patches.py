...
import json
sys.path.append('../')
from utils import downPixelAggr, save_test_patches, save_random_patches, save_random_patches60, save_test_patches60
...
def readS2fromFile(data_file,
    ...
    if test_data:
        if run_60:
            ...
            out_per_image0 = save_prefix + 'test32_bicubic/'
            out_per_image = save_prefix + 'test32_bicubic/' + data_file + '/'
        ...
        if run_60:
            ...
            save_test_patches(data10_lr, data20_lr, out_per_image, patchSize=32)
            with open(out_per_image + 'roi.json', 'w') as f:
        ...
        if run_60:
            ...
            out_per_image0 = save_prefix + 'train32_bicubic/'
            out_per_image = save_prefix + 'train32_bicubic/' + data_file + '/'
        if not os.path.isdir(out_per_image0):
            os.mkdir(out_per_image0)
        if not os.path.isdir(out_per_image):
            os.mkdir(out_per_image)
