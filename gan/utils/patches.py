...
def get_test_patches_(dset_10, dset_20, dset_10_gt, dset_20_gt, patchSize=128, border=4, interp=True):
    ...
    PATCH_SIZE_HR_10 = (patchSize*2, patchSize*2)
    BORDER_HR_10 = BORDER_HR*2
    ...
    dset_20_gt = np.pad(dset_20_gt, ((BORDER_HR, BORDER_HR), (BORDER_HR, BORDER_HR), (0, 0)), mode='symmetric')
    dset_10_gt = np.pad(dset_10_gt, ((BORDER_HR_10, BORDER_HR_10), (BORDER_HR_10, BORDER_HR_10), (0, 0)), mode='symmetric')
    ...
    label_10 = np.zeros((nr_patches, BANDS10) + PATCH_SIZE_HR_10).astype(np.float32)
    ...
    for ii in range_i.astype(int):
        for jj in range_j.astype(int):
            ...
            crop_point_hr_10 = [p*2 for p in crop_point_hr]
            ...
            label_20[pCount] = np.rollaxis(dset_20_gt[crop_point_hr[0]:crop_point_hr[2],
                             crop_point_hr[1]:crop_point_hr[3]], 2)
            label_10[pCount] = np.rollaxis(dset_10_gt[crop_point_hr_10[0]:crop_point_hr_10[2],
                             crop_point_hr_10[1]:crop_point_hr_10[3]], 2)                 
            pCount += 1
    ...
    if interp:
        ...
        label20_interp = interp_patches(label_20, label_10.shape)
    else:
        data20_interp = image_20
    return image_10, data20_interp, label_10, label_20, label20_interp
...
def get_test_patches60(dset_10, dset_20, dset_60, patchSize=128, border=8, interp=True):
    ...
    #dset_60_gt = np.pad(dset_60_gt, ((BORDER_10, BORDER_10), (BORDER_10, BORDER_10), (0, 0)), mode='symmetric')
    ...
    #image_60_gt = np.zeros((nr_patches, BANDS60) + tuple(PATCH_SIZE_10)).astype(np.float32)
    # print(image_10.shape)
    # print(image_20.shape)
    ...
    for ii in range_i.astype(int):
        for jj in range_j.astype(int):
            ...
            #image_60_gt[pCount] = np.rollaxis(dset_60_gt[crop_point_10[0]:crop_point_10[2],
            #                                 crop_point_10[1]:crop_point_10[3]], 2)                                 
            pCount += 1
...
def save_test_patches_(dset_10, dset_20, dset_10_gt, dset_20_gt, file, patchSize=128, border=4, interp=True):
    image_10, data20_interp, data10_gt, data20_gt, label_20_interp = get_test_patches(dset_10, dset_20, dset_10_gt, dset_20_gt, patchSize=patchSize, border=border, interp=interp)

    print("Saving to file {}".format(file))

    np.save(file + 'data10', image_10)
    np.save(file + 'data20', data20_interp)
    np.save(file + 'data20_gt', data20_gt)
    np.save(file + 'data10_gt', data10_gt)
    np.save(file + 'data20_gt_interp', label_20_interp)
    print('Done!')
...
def save_test_patches(dset_10, dset_20, file, patchSize=128, border=4, interp=True):
    ...
    np.save(file + 'data10_gt', image_10)
    np.save(file + 'data20_gt_interp', data20_interp)
    print('Done!')
    
def save_test_patches60(dset_10, dset_20, dset_60, file, patchSize=96, border=12, interp=True):

    image_10, data20_interp, data60_interp = get_test_patches60(dset_10, dset_20, dset_60, patchSize=patchSize, border=border, interp=interp)
    ...
    #np.save(file + 'data10_gt', image_10_gt)
    #np.save(file + 'data20_gt_interp', data20_gt_interp)
    #np.save(file + 'data60_gt_interp', data60_gt_interp)
    
    #np.save(file + 'data60_gt', data_60_gt)
    print('Done!')

def save_test_patches60_(dset_10, dset_20, dset_60, file, patchSize=192, border=12, interp=True):

    image_10, data20_interp, data60_interp = get_test_patches60(dset_10, dset_20, dset_60, patchSize=patchSize, border=border, interp=interp)
    print("Saving to file {}".format(file))

    np.save(file + 'data10', image_10)
    np.save(file + 'data20_interp', data20_interp)
    np.save(file + 'data60_interp', data60_interp)
    print('Done!')
...
def save_random_patches_64_32(dset_20gt, dset_10, dset_20, file, NR_CROP=2000):
    ...
    print('Done!')
"""  
def get_test_patches60_lanczos4(dset_10, dset_20, dset_60, patchSize=128, border=8, interp=True):

    dset_20 = cv2.resize(dset_20, (dset_60gt.shape[0], dset_60gt.shape[1]),  interpolation = cv2.INTER_LANCZOS4)
    dset_60 = cv2.resize(dset_60, (dset_60gt.shape[0], dset_60gt.shape[1]),  interpolation = cv2.INTER_LANCZOS4)
    PATCH_SIZE_10 = (patchSize, patchSize)
    PATCH_SIZE_20 = [p//2 for p in PATCH_SIZE_10]
    PATCH_SIZE_60 = [p//6 for p in PATCH_SIZE_10]
    BORDER_10 = border
...
def save_random_patches60_lanczos4(dset_60gt, dset_10, dset_20, dset_60, file, NR_CROP=500):
    dset_20 = cv2.resize(dset_20, (dset_60gt.shape[0], dset_60gt.shape[1]),  interpolation = cv2.INTER_LANCZOS4)
    dset_60 = cv2.resize(dset_60, (dset_60gt.shape[0], dset_60gt.shape[1]),  interpolation = cv2.INTER_LANCZOS4)
    PATCH_SIZE_60 = (96, 96)
    ...
    label_60 = np.zeros((NR_CROP, 96, 96, BANDS60)).astype(np.float32)
    image_10 = np.zeros((NR_CROP, 96, 96, BANDS10)).astype(np.float32)
    image_20 = np.zeros((NR_CROP, 96, 96, BANDS20)).astype(np.float32)
    image_60 = np.zeros((NR_CROP, 96, 96, BANDS60)).astype(np.float32)
    ...
    for crop in range(0, NR_CROP):
        ...
        label_60[i] = dset_60gt[crop_point_lr[0]:crop_point_lr[2], crop_point_lr[1]:crop_point_lr[3]]
        image_10[i] = dset_10[crop_point_lr[0]:crop_point_lr[2], crop_point_lr[1]:crop_point_lr[3]]
        image_20[i] = dset_20[crop_point_lr[0]:crop_point_lr[2], crop_point_lr[1]:crop_point_lr[3]]
        image_60[i] = dset_60[crop_point_lr[0]:crop_point_lr[2], crop_point_lr[1]:crop_point_lr[3]]
        i += 1
...
def downPixelAggr60(img, SCALE=2):
    from scipy import signal
    import skimage.measure
    from scipy.ndimage.filters import gaussian_filter
    if len(img.shape) == 2:
    ...
    # Filter the image with a Gaussian filter
    for i in range(0, img.shape[2]):
        img_blur[:, :, i] = gaussian_filter(img[:, :, i], 1/SCALE, truncate=12.0)
    # New image dims
    new_dims = tuple(s//SCALE for s in img.shape)
    img_lr = np.zeros(new_dims[0:2]+(img.shape[-1],))
    # Iterate through all the image channels with avg pooling (pixel aggregation)
