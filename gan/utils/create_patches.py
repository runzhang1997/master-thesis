...
import json
sys.path.append('../')
from utils.patches import downPixelAggr, downPixelAggr60, save_test_patches, save_test_patches_, save_random_patches, save_random_patches_64_32, save_random_patches60, save_test_patches60_, save_test_patches60
...
def readS2fromFile(data_file,
    ...
    if not true_data:
        if run_60:
            ...
            data10_lr=downPixelAggr60(data10_gt, SCALE=scale60)
            data20_lr=downPixelAggr60(data20_gt, SCALE=scale60)
            data60_lr=downPixelAggr60(data60_gt, SCALE=scale60)
        else:
    ...
    if test_data:
        if run_60:
            out_per_image0 = save_prefix + 'test60_96_interp_new_degradation/'
            out_per_image = save_prefix + 'test60_96_interp_new_degradation/' + data_file + '/'
        else:
            # modify add 32 to generate 32 test
            out_per_image0 = save_prefix + 'test20_32_interp_bilinear/'
            out_per_image = save_prefix + 'test20_32_interp_bilinear/' + data_file + '/'
        ...
        if run_60:
            save_test_patches60_(data10_lr, data20_lr, data60_lr, out_per_image, patchSize=96, border=12, interp=True)
            with open(out_per_image + 'roi.json', 'w') as f:
                json.dump([tmxmin // scale60, tmymin // scale60, (tmxmax + 1) // scale60, (tmymax + 1) // scale60], f)
        else:
            save_test_patches(data10_lr, data20_lr, data10_gt, data20_gt, out_per_image,patchSize=32, border=4, interp=True)
            with open(out_per_image + 'roi.json', 'w') as f:
        ...
        if run_60:
            ...
            # comment the following three lines because do not have
        ...
        if run_60:
            out_per_image0 = save_prefix + 'train60_96_new_degradation/'
            out_per_image = save_prefix + 'train60_96_new_degradation/' + data_file + '/'
        else:
            out_per_image0 = save_prefix + 'train20_128/'
            out_per_image = save_prefix + 'train20_128/' + data_file + '/'
        if not os.path.isdir(out_per_image0):
            os.mkdir(out_per_image0)
        if not os.path.isdir(out_per_image):
            os.mkdir(out_per_image)
