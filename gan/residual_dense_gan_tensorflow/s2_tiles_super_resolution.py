...
discriminator = network.Discriminator(loss_type = 'wgan-gp', image_size=args.patch_size,learning_rate=0.0005)
generator = network.Generator(adversarial_loss ='wgan-gp', discriminator=discriminator, learning_rate=0.0005,predict=True)
# Generatoradversarial_loss
...
if validated_10m_bands and validated_20m_bands:
    ...
    border = 8
    p10, p20 = get_test_patches(data10, data20, patchSize=128, border=border)
    p10 /= SCALE
    p20 /= SCALE
    test = [p10, p20]
    ...
    with tf.Session() as sess:
        ...
        saver.restore(sess, args.model)
            ...
            for i in range(0,229):
                prediction_i = sess.run(g_y_pred, feed_dict={input_1: train10[i*229:(i+1)*229],input_2: train20[i*229:(i+1)*229]})
                if i ==0:
